...
function bindControls() {
  ...
  byID("game-encounter-collect")?.addEventListener("click", () => {
  ...
  const detailCard = byID("game-card-detail-card");
  ...
  byID("game-card-detail-close")?.addEventListener("click", closeCardDetail);
  byID("game-card-detail")?.addEventListener("click", (event) => {
...
async function commitSliderInputValue(scope, input) {
  ...
  try {
    ...
    if (scope === "library") {
      ...
      if (!cardId2) return;
      applyGameResult(await applyGameLibraryComponentControl(cardId2, componentId, "slider", "value", value));
      return;
    }
...
function activeCardPreview() {
  const detail = byID("game-card-detail");
  if (detail && !detail.hidden) {
    const expanded = byID("game-card-detail-card")?.firstElementChild;
...
async function submitActiveForm(form) {
  if (busy) return;
  const cardId = form.closest("[data-card-id]")?.dataset.cardId || activeCardPreview()?.dataset.cardId || latestSession?.activeWorldCardId || "";
  ...
  busy = true;
  const submitButtons = Array.from(form.querySelectorAll("button[data-card-form-control]"));
...
function applyGameResult(result, options = {}) {
...
function presentGameEvents(events, snapshot) {
  const message = events.slice().reverse().find((event) => event.type === "message")?.message;
...
function renderEncounter(encounter) {
  ...
  const root = byID("game-encounter");
  const opposition = byID("game-field-opposition");
  const player = byID("game-field-player");
  ...
  if (!participantIds.has(focusedFieldCardId)) {
    focusedFieldCardId = participantIds.has(latestSession?.activeWorldCardId || "") ? latestSession.activeWorldCardId : encounter.participants[0]?.card.id || "";
  }
  const title = byID("game-encounter-title");
  const phase = byID("game-encounter-phase");
  if (title) title.textContent = humanize(encounter.id);
  if (phase) phase.textContent = encounter.phase === "resolved" && encounter.outcome ? `Resolved \xB7 ${humanize(encounter.outcome)}` : humanize(encounter.phase);
  renderPressure(encounter);
  encounter.participants.forEach((participant) => {
    const lane = participant.role === "hostile" || participant.role === "environmental" ? opposition : player;
...
function renderPressure(encounter) {
  const value = byID("game-pressure-value");
  const fill = byID("game-pressure-fill");
...
function renderFieldCard(participant) {
  ...
  [participant.role, participant.card.actor?.disposition, ...(participant.card.actor?.statuses || []).slice(0, 2)].filter((value) => Boolean(value)).forEach((value) => {
...
function bindFieldCardInteractions(tile, participant) {
  let pointerId = -1;
  let startX = 0;
  let startY = 0;
  let timer = 0;
...
function focusFieldCard(cardId) {
  focusedFieldCardId = cardId;
  document.querySelectorAll(".game-field-card").forEach((tile) => {
...
function renderActorTrack(label, resource) {
  const row = document.createElement("div");
...
function openCardDetail(participant, focus = true) {
  const root = byID("game-card-detail");
  const cardRoot = byID("game-card-detail-card");
  const title = byID("game-card-detail-title");
  const role = byID("game-card-detail-role");
  ...
  root.hidden = false;
  if (focus) byID("game-card-detail-close")?.focus({ preventScroll: true });
}
...
function setStatus(message, isError = false) {
  ...
  const encounterStatus = byID("game-encounter-status");

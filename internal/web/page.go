...
func designerControlsView() *Node {
	return Div(
		...
		Form(
			...
			Div(
				Class("space-y-2"),
				Label(Attr("for", "config-target"), Class("block text-[0.72rem] font-semibold uppercase text-[var(--app-fg-soft)]"), T("Config Kind")),
				Select(
					Id("config-target"),
					Name("componentKind"),
					Class(uiInputClass()),
			...
			Div(
				Class("space-y-2"),
				Label(Attr("for", "config-instruction"), Class("block text-[0.72rem] font-semibold uppercase text-[var(--app-fg-soft)]"), T("Instruction")),
				TextArea(
					Id("config-instruction"),
					Name("instruction"),
					Class(uiInputClass()+" min-h-28 resize-y"),
					Placeholder("Describe the config you want..."),
				),
			),
			Div(
		...
		Div(
			...
			H3(Class("text-[0.72rem] font-semibold uppercase text-[var(--app-fg-soft)]"), T("Generated config")),
			P(Id("config-description"), Class("text-sm text-[var(--app-fg-muted)]"), T("No generated config yet.")),
			TextArea(
				Id("config-preview"),
